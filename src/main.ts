...
function getActiveEnsembleSummary(project: StoryProject, activeScene?: Scene) {
  if (!activeScene || activeScene.characterIds.length < 3) return undefined

  const cast = activeScene.characterIds
    .map((id) => characterById(project, id))
    .filter((character): character is Character => Boolean(character))

  if (cast.length < 3) return undefined

  const pairs = cast.flatMap((character, index) =>
    cast.slice(index + 1).map((other) => ({
      ids: [character.id, other.id].sort().join('|'),
      names: `${character.name} and ${other.name}`,
    })),
  )

  const pairCount = pairs.length
  const pairIds = new Set(pairs.map((pair) => pair.ids))
  const mostConnectedCount = cast.length - 1
  const mostConnected = cast.map((character) => character.name)
...
function renderMeetings(project: StoryProject, scenes: Scene[]) {
  ...
  const ensemble = getActiveEnsembleSummary(project, activeScene)
  const pairs = new Map<string, { ids: string; names: string; count: number; moments: string[]; active: boolean }>()
    ...
    for (let i = 0; i < scene.characterIds.length; i += 1) {
      for (let j = i + 1; j < scene.characterIds.length; j += 1) {
        ...
        if (existing) {
          ...
          pairs.set(ids, { ids, names: label, count: 1, moments: [moment], active: isActivePair || isCastPair })
        }
      }
    }
  }
  ...
  return `
    <section class="panel meetings-panel">
      ...
      ${ensemble && activeScene ? `
        <div class="panel-subtle ensemble-summary active-arrival">
          <div class="section-head ensemble-summary-head">
            <div>
              <p class="eyebrow">Active ensemble</p>
              <h3>${escapeHtml(activeScene.title || 'Untitled scene')}</h3>
              <p class="muted ensemble-scene-meta">${escapeHtml([
                activeScene.timeLabel || '',
                locationName || '',
              ].filter(Boolean).join(' · ') || 'No time or location set')}</p>
            </div>
            <button class="inline-action" data-action="open-scene-from-meetings" data-scene-id="${activeScene.id}" data-chapter-id="${activeScene.chapterId}">Return to scene editor</button>
          </div>
          <div class="stats-grid ensemble-stats">
            <div><span>Core cast</span><strong>${ensemble.cast.length}</strong><p class="muted">${escapeHtml(ensemble.cast.map((character) => character.name).join(', '))}</p></div>
            <div><span>Pair coverage</span><strong>${ensemble.pairCount}</strong><p class="muted">Distinct pair cards generated by this scene.</p></div>
            <div><span>Most connected characters</span><strong>${ensemble.mostConnectedCount}</strong><p class="muted">${escapeHtml(ensemble.mostConnected.join(', '))}</p></div>
            <div><span>Meetings view status</span><strong>Ensemble</strong><p class="muted">Active-scene pairs are highlighted first below.</p></div>
          </div>
          <div>
            <p class="eyebrow">Core cast</p>
            <div class="tag-row">${ensemble.cast.map((character) => `<span>${escapeHtml(character.name)}</span>`).join('')}</div>
          </div>
        </div>
      ` : ''}
